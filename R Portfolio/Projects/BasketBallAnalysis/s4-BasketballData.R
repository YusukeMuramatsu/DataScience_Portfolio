...
legend("bottomleft",inset=0.05, legend=Players,col=c(1:4,6),pch=15:18,horiz=F)
...
Games #this is matrix
Games[1:3,6:10]
Games[c(1,10),]
Games[,c("2008","2009")]

#As access to matrix, data could change to vector.
#Here is how you change and not change data type
Games[1,]  # this is vector
Games[1,,drop=F] # this is matrix. R does not DROP data.
Games[1,5] # this is vector
Games[1,5,drop=F] # this is matrix.


Data <- MinutesPlayed[1:3,]
matplot(t(Data), type = "b", pch=15:18, col=c(1:4,6))
legend("bottomleft",inset=0.1, legend=Players[1:3],col=c(1:4,6),pch=15:18,horiz=F)

#Function Example1
myplot <- function(){
  Data <- MinutesPlayed[1:5,]
  matplot(t(Data), type = "b", pch=15:18, col=c(1:4,6))
  legend("bottomleft",inset=0.1, legend=Players[1:5],col=c(1:4,6),pch=15:18,horiz=F)
  }
myplot()
#Function Example2
...
#Function Example3 Taking 2 vars
myplot <- function(data,rows){
  Data <- data[rows,]
  matplot(t(Data), type = "b", pch=15:18, col=c(1:4,6))
  legend("bottomleft",inset=0.1, legend=data[rows],col=c(1:4,6),pch=15:18,horiz=F)
}
myplot(Salary,1:2)

#Function Example4 Taking 2 vars
